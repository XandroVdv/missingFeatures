export default function(args) {
    ChatLib.chat("&6[---------------------------------------------------]");
    ChatLib.chat("&6[MissingFeatures] &7Available commands:");
    ChatLib.chat("&r&e/mf toggleInventory &7- Toggle the inventory display.");
    ChatLib.chat("&r&e/mf setInventoryCorner <corner> &7- Set the corner of the inventory display.");
    ChatLib.chat("&r&e/mf toggleArmor &7- Toggle the armor display.");
    ChatLib.chat("&r&e/mf toggleGoldenGoblinMessage &7- Toggle the Golden Goblin")
    ChatLib.chat("&r&e/mf togglePristineMessage &7- Toggle Pristine spam.");
    ChatLib.chat("&r&e/mf toggleAvgPristine &7- Toggle the average pristine display.");
    ChatLib.chat("&r&e/mf help &7- Display this help message.");
    ChatLib.chat("&6[---------------------------------------------------]");
}
